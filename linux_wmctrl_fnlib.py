...
def get_client_stacking():
  ...
  """
    outp_byte = subprocess.check_output(
        ["xprop", "-root"]
...
def get_active_window_id():
  ...
  """
    ...
    win_id_byte = subprocess.check_output(
...
def get_active_windows(sort=True):
  ...
  """
    ...
    dat_client_stack = pd.DataFrame(
        data={"win_id": client_stack, "active_rank": reversed(range(len(client_stack)))}
    )
    ## join to wmctrl data:
    dat_wmctrl = pd.merge(
        dat_wmctrl, dat_client_stack, left_on="win_id", right_on="win_id"
    )
