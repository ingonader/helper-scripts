...
def get_active_window_id():
  ## get active window ID via xprop:
  win_id_byte = subprocess.check_output(["/usr/bin/xprop", "-root", "_NET_ACTIVE_WINDOW"])
  win_id_str = win_id_byte.decode("utf-8")
  ## also possible: subprocess.check_call; subprocess.call;
  ## don't use os.system any longer:
  ## # win_id = os.system("xprop -root _NET_ACTIVE_WINDOW")
  
  ## extract active window ID from results string:
  #re.search(r'.*?: window id # ', win_id_str)
  win_id = win_id_str
...
## note: function returns pandas dataframe without an index without any information (index only contains only an enumeration)
def get_wmctrl_data():
